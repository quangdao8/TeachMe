...
class Drawer extends React.Component {
      ...
      if (response === grated) {
        Permissions.check(CAMERA).then(response => {
          if (response == grated) {
            ...
            alert(
              I18n.t('Alert.notPermission') + 'camera' + I18n.t('Alert.forApp'),
            );
          } else {
  ...
  async UNSAFE_componentWillMount() {
    ...
    // });
    console.log('llllllllllllllllll', Platform.OS);
    this.checkPermission(MICROPHONE);
    console.log('llllllllllllllllll 2222222', Platform.OS);

    if (Platform.OS === 'android') {
      console.log('llllllllllllllllll 333333', this.state.userRe.data.id);
    ...
    switch (type) {
      case 1:
        return (
          <TouchableOpacity
            style={{flex: 1}}
            onPress={() => this.navigate('MyQrCode')}>
            <AppImage
              local
              source={ICON.IC_QRCODE}
