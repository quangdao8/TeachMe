...
class VideoCall extends Component {
    ...
    constructor(props) {
        ...
        const { isSpeaker = false, isMute = false, callLogId, callData, isVideo, isFrom } = params;
        this.state = {
            videoURL: "",
            isFront: true,
            friendVideo: "",
        ...
        isFrom &&
            Platform.OS === "ios" &&
            RNCallKeep.startCall(this.uuid, callData.callerName, callData.callerName, "generic", isVideo);
    }
    isDisconnect = false;
    isSendIce = false;
    ...
    _handleAppStateChange = nextAppState => {
        if (Platform.OS !== "ios") {
            ...
            if (this.appState.match(/background/) && nextAppState === "active") {
                console.log("thisvvvvvvvvvvvvvvvvvvvvvvvvv", this.appState, nextAppState);
                if (!!this.timeoutId) {
    ...
    componentDidMount() {
        ...
        const { params = {} } = this.props.navigation.state;
        const { isFrom, userId, roomId, note = "", callerId, callData, isVideo } = params;
        ...
        if (Platform.OS !== "ios") {
            ...
            if (isFrom) {
                ...
                setTimeout(() => {
                    RNCallKeep.updateDisplay(this.uuid, callData.callerName, callData.callerName);
                }, 1000);
                            ...
                            userActions.getUserData({ id }).then(res => {
                                ...
                                if (!res.error) {
                                    ...
                                    this.setState({
                                        ...
                                        duration: moment(
                                            this.timer > defaultTimer ? this.timer * 1000 : defaultTimer * 1000
                                        )
                                            .utc()
                                            .format("HH:mm:ss"),
                                        price: this.price,
