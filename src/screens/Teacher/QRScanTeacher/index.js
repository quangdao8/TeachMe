...
import { Dimensions, View, Alert, Vibration } from "react-native";
import { connect } from "react-redux";
import { Const, GlobalStyles, Helper, ServiceHandle, Colors } from "helper";
...
class QRScanTeacher extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            focusedScanner: "",
        ...
        if (prevProps.contactReducer !== contactReducer && !reactivate) {
            if (contactReducer.type == types.FINDER_SUCCESS) {
                if (
                    ...
                    contactReducer.dataFinder.results[0].type == 1
                ) {
                    this.setState({ reactivate: true }, () => {
                        navigation.navigate("DetailTeacher", {
                            contact: { teacher: contactReducer.dataFinder.results[0] }
                        });
                    });
                    return;
                } else {
                    setTimeout(() => {
                        // Alert.alert(I18n.t("Alert.notice"), I18n.t("Alert.cantFindPhone"));
                        const paramsAlert = {
                    ...
                    return;
                    // Alert.alert("Thông báo", "Không tìm thấy số điện thoại", [Ơ]);
                }
            }
    ...
    onSuccess = e => {
        ...
        const { reactivate, vibration } = this.state;
        !vibration && Vibration.cancel();
    ...
    render() {
        ...
        return (
            <Container scrollEnabled={false}>
                <HeaderApp title={I18n.t("QRScan.QRScan")} isBack navigation={this.props.navigation} />
...
QRScanTeacher = connect(mapStateToProps)(QRScanTeacher);
export default QRScanTeacher;
