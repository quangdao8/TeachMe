...
const answerCall = handler => eventEmitter.addListener(RNCallKeepPerformAnswerCallAction, data => handler(data));

const endCall = handler => eventEmitter.addListener(RNCallKeepPerformEndCallAction, data => handler(data));

const didActivateAudioSession = handler => eventEmitter.addListener(RNCallKeepDidActivateAudioSession, handler);

const didDeactivateAudioSession = handler => eventEmitter.addListener(RNCallKeepDidDeactivateAudioSession, handler);
...
const didToggleHoldCallAction = handler => eventEmitter.addListener(RNCallKeepDidToggleHoldAction, handler);

const didPerformDTMFAction = handler => eventEmitter.addListener(RNCallKeepDidPerformDTMFAction, data => handler(data));

const didResetProvider = handler => eventEmitter.addListener(RNCallKeepProviderReset, handler);

const checkReachability = handler => eventEmitter.addListener(RNCallKeepCheckReachability, handler);
